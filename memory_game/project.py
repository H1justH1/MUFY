...
# Enhanced CSS with animations and mobile responsiveness - CLICKABLE EMOJI CARDS
css_styles = f"""
<style>
...
/* Clickable card styling - unified emoji and click area */
.card-clickable {{
...
@media (max-width: 768px) {{
    .card-clickable, .card-flip, .card-match {{
...
with header_container:
    ...
    with st.expander("📜 Game Rules", expanded=False):
        ...
        st.markdown("""
        - Two or more players take turns clicking cards.
...
# Game board with clickable emoji cards
with game_board_container:
    ...
    for i in range(rows * cols):
        with cols_streamlit[i % cols]:
            if game["matched"][i]:
                # Matched card - show emoji with match styling (not clickable)
                st.markdown(f'<div class="card-clickable card-match card-disabled">{game["deck"][i]}</div>', 
                          unsafe_allow_html=True)
            elif game["flipped"][i]:
                # Flipped card - show emoji with flip animation (not clickable)
                st.markdown(f'<div class="card-clickable card-flip card-disabled">{game["deck"][i]}</div>', 
                          unsafe_allow_html=True)
            else:
                # Unflipped card - clickable emoji box
                if not game["waiting"]:
                    # Create a clickable card with hidden button for Streamlit interaction
                    card_content = "❓"
                    card_classes = "card-clickable card-unflipped"
                    
                    # Use columns to overlay button on the card
                    card_col = st.container()
                    with card_col:
                        st.markdown(f'<div class="{card_classes}">{card_content}</div>', 
                                  unsafe_allow_html=True)
                        
                        # Hidden button that overlays the card
                    ...
                    # Waiting state - show disabled card
                    st.markdown(f'<div class="card-clickable card-unflipped card-disabled">❓</div>', 
                              unsafe_allow_html=True)

# Add custom CSS to hide the button and make the card area clickable
st.markdown("""
<style>
/* Hide the actual streamlit buttons but keep functionality */
